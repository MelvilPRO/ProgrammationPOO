...
var sectionExo = document.querySelector("#exo1");
for (let equipeId = 0; equipeId < equipes.length; equipeId++){
    let equipe = equipes[equipeId];

    for (let personne of equipe){ 
        let nouveauUl = document.createElement("ul");
        nouveauUl.innerText = personne.toString();
        sectionExo.appendChild(nouveauUl)
    }
}
